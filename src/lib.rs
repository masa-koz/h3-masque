...
use h3::{client::RequestStream as ClientRequestStream, quic::BidiStream, server::RequestStream};
use h3::{
        ...
        let request = async move {
            ...
            let req = http::request::Request::builder()
                ...
                .uri(format!(
                    "https://{}/{}",
                    server_addr,
                    socketaddr_to_connect_udp_path(&Some(remote_addr))
                ))
...
pub async fn open_udp_bind_proxy(
        ...
        let request = async move {
            ...
            let req = http::request::Request::builder()
                ...
                .uri(format!(
                    "https://{}/{}",
                    server_addr,
                    socketaddr_to_connect_udp_path(&target_addr)
                ))
            ...
            let req_length = encode_var_int(0x11).len() + 1 + encode_var_int(context_id).len() + 1;
            req_buf.extend_from_slice(
                &encode_var_int(0x11), // COMPRESSION capsule
            );
            req_buf.extend_from_slice(&encode_var_int(req_length as u64));
            req_buf.extend_from_slice(&encode_var_int(context_id));
            ...
            Some(data) => {
                ...
                match capsule_type {
                    0x12 | 0x13 => {
                        ...
                        let Some((context_id, payload)) = decode_var_int(payload) else {
    ...
    H: SendDatagram<Bytes> + 'static + Send,
{
    tokio::spawn(async move {
        ...
        let stream_id = stream.id();
        let handle_capsule = run_server_process_capsule(stream, sessions, context_info.clone(), stream_id.clone());
 ...
 */
fn socketaddr_to_connect_udp_path(addr: &Option<SocketAddr>) -> String {
    if let Some(addr) = addr {
